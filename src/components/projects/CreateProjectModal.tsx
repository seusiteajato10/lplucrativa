...
const CreateProjectModal = ({ open, onOpenChange }: CreateProjectModalProps) => {
  ...
  const { limits, loading: subscriptionLoading, canCreateProject } =
    useSubscription();
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Escolha o tipo de página que deseja criar hoje.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-2">
            ...
            >
              ...
              <div className="relative">
                ...
                >
                  ...
                  <Badge
                    variant="outline"
                    className="mt-1 text-[8px] h-3 px-1 border-primary/30 text-primary"
                  >
                    RECOMENDADO
                  </Badge>
                </Label>
          ...
          <div className="space-y-2">
            ...
            <Select
              value={niche}
              onValueChange={(v) => setNiche(v as ProjectNiche)}
            >
              <SelectTrigger>
                <SelectValue placeholder="Selecione" />
              </SelectTrigger>
              <SelectContent>
                {Object.entries(nicheLabels).map(([k, l]) => (
          ...
          <div className="space-y-3">
                ...
                {currentTemplateOptions.map((tpl) => (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-3 rounded-lg border-2 transition-all gap-1 ${
                      selectedTemplateId === tpl.value
                        ? "border-primary bg-primary/5"
                        : "border-muted hover:border-border"
                    }`}
                  >
                    {tpl.value === "capture_ebook" && (
                      <BookOpen className="w-5 h-5 mb-1" />
                    )}
                    {tpl.value === "capture_vsl" && (
                      <PlayCircle className="w-5 h-5 mb-1" />
                    )}
                    {tpl.value === "capture_quiz" && (
                      <HelpCircle className="w-5 h-5 mb-1" />
                    )}
                    {tpl.value === "capture_discount" && (
            ...
            {errors.template && (
              <p className="text-[10px] text-destructive">{errors.template}</p>
            )}

            {/* Link para ver galeria de templates */}
            <div className="flex items-center justify-end">
              <button
                type="button"
                onClick={() => window.open("/templates", "_blank")}
                className="flex items-center gap-1 text-[11px] text-primary underline underline-offset-2 hover:opacity-80"
              >
                <Layout className="w-3 h-3" />
                Ver templates em tela cheia
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ex: E-book Fone Pro"
            ...
            <div className="space-y-2">
              ...
              {errors.slug && (
                <p className="text-[10px] text-destructive">{errors.slug}</p>
              )}
            </div>
          </div>
          ...
          {!canCreate && (
            <Alert variant="destructive">
              ...
              <AlertDescription className="text-[11px]">
                Você atingiu seu limite. Faça upgrade para criar mais projetos.
              </AlertDescription>
            </Alert>
          )}
